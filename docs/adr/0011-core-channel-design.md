...
**决策**：取**最小复杂度方案为骨架**，移植**性能方案的四处中的两处**——`EventSink` 的 try/await 双路径、`Inbox = 日志 + 游标`。**不采纳**可扩展方案的 `EventBus`/`Projection`/`SharedLog`/`InboundHub`/`TurnDriver`（它们服务于已被推迟的群聊/多消费者）。
...
3. **`Inbox = 日志 + 游标 + Intent 队列`**，落实 context.md 已定语义。`/new` = 换一条 `log`。
4. **自转接口 = `Agent::run(&mut self, inbox, policy)`**，复用现有 `Agent`；`run_turn` 降级为内部「跑一个回合」的原语（ADR-0010 留空的正是这块）。
5. **`LifecyclePolicy` 用 enum + `#[non_exhaustive]`**，非 trait——两种策略已定，第三种加变体即可。
6. **agent 不休眠**：`run()` 在 inbox 空时返回，接线器按需重驱动。省掉 `Notify`/常驻任务/唤醒协议。
7. **ADR-0010 的所有权收敛（session/events 移出 `Agent`）排到迁移步 5**，与信道解耦，可最后做或不做。

## Consequences

- **信道不是瓶颈**：`Envelope` 96 B、流式 20–200 events/s、单事件 30–80 ns、**~0.02% CPU**。故不做生产者侧批处理（那会破坏「增量拼接 == 终态」不变量）。
